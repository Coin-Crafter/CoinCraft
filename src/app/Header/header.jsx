...
//     // save filteredProjects.length to firebase as "Projects Completed"
//     // const docRef = doc(db, "profiles", freelancerWallet.toLowerCase());
//     // await setDoc(docRef, 
//     //   { 
//     //     projectsCompleted: filteredProjects.length, 
//     //     walletAddress: freelancerWallet.toLowerCase(), 
//     //     profilePicture: profileDetails.profilePicture,
//     //     name: profileDetails.name,
//     //     description: profileDetails.description,
//     //     githubLink: profileDetails.githubLink,
//     //     linkedinLink: profileDetails.linkedinLink
//     //   });
...
const Header = () => {
  ...
  const [editableProfileDetails, setEditableProfileDetails] = useState({ ...profileDetails });
  ...
  const saveProfile = async () => {
    // Check if name is empty again before saving
    if (!editableProfileDetails.name.trim()) {
      alert("Name cannot be empty.");
      return;
    }

    if (editableProfileDetails.profilePicture && !isValidImageURL(editableProfileDetails.profilePicture)) {
      // Since we now store data URLs, this validation might not be necessary
      // But if we still want to ensure image format, you can skip this
      // For now, let's remove this validation since we store a data URL.
    }
    ...
    try {
      ...
      console.log("Profile updated:", editableProfileDetails);
      setIsEditModalOpen(false); // Close the modal
      setIsProfileOpen(false); // Close the profile modal if open
    } catch (error) {
      console.error("Error saving profile details:", error);
  ...
  const handleImageChange = (e) => {
    ...
    reader.onload = () => {
      const dataUrl = reader.result;
      setEditableProfileDetails((prev) => ({
        ...prev,
        profilePicture: dataUrl,
      }));
    };
  ...
  // Open edit modal and set editable profile details
  const openEditModal = () => {
        ...
        <div className="modal">
          <div className="modal-content">
            ...
            <input
              ...
              name="githubLink"
              placeholder="GitHub Link"
              value={editableProfileDetails.githubLink}
              onChange={handleInputChange}
            />
            <input
              type="url"
              name="linkedinLink"
              placeholder="LinkedIn Link"
              value={editableProfileDetails.linkedinLink}
              onChange={handleInputChange}
            />
            <div className="modal-buttons">
              <button onClick={saveProfile}>Save</button>
